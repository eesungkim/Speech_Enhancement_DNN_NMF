...
def DNN_TF_Masking(args):
    ...
    dnn_magnitude_noisy_train                   = apply_context_window(dnn_magnitude_noisy_train.T, args.left_context, args.right_context).T
    ...
    dnn_magnitude_noisy_test_supervector        = apply_context_window(dnn_magnitude_noisy_test.T, args.left_context, args.right_context).T

    X_train = np.log(dnn_magnitude_noisy_train.T**2)
    y_train = estimate_IRM(dnn_magnitude_clean_train.T,dnn_magnitude_noise_train.T)
    X_test  = np.log(dnn_magnitude_noisy_test_supervector.T**2)
    ...
    # Enhancement stage
    #####################################################################################
    speech_masks = model.predict(X_test).T
    magnitude_estimated_clean = dnn_magnitude_noisy_test * speech_masks
...
def parse_args():
    ...
    parser.add_argument('--output_file',        type=str, default='datasets/output/estimated_clean_DNN-TF-Masking.wav')
    parser.add_argument('--left-context', type=int, dest="left_context", default=0, help="left context of inputs for neural networks")
    parser.add_argument('--right-context', type=int, dest="right_context", default=0, help="right context of inputs for neural networks")
    ...
    parser.add_argument('--n_epoch',    type=int, default='30', help='number of DNN epoch')
